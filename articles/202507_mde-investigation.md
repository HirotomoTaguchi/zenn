...
Microsoft Defender for Endpoint (MDE) で不審なファイルを見る場合、何をどう見たらいいのかを自分のためにもまとめます。

## 前提条件

**対象読者**

- MDEの管理者であるが、あまりインシデント対応に明るくない方

**記載すること**

- Microsoft Defender for Endpoint (MDE) P2ライセンスの範囲でできる、不審なファイルを見る場合の代表的な見るべきポイント

**記載しないこと**

- 不審なファイル以外のシナリオにおいて、Microsoft Defender XDR で検出されたアラート（Microsoft Defender XDRでは、不審なファイルだけではなく、不審なPowerShellの実行やIdentity関連の検出も含まれます）
- 一般的なデジタルフォレンジックの話やマルウェア解析の話（あくまでMDEに閉じた話をします）

## 全体像

### 調査アプローチ

調査においては、広い入口から絞り込んで原因を特定し（ファネル）、その特定した原因を基に今度は影響範囲を広げて調査するという砂時計のようなアプローチを大事にしています。

1. 初期アラート（広い視点）
2. ファイル分析（焦点を絞る）
3. 組織全体への影響評価（再び視野を広げる）
